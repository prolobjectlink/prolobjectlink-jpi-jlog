...
public class JLogTypedResult extends JLogResult implements PrologTypedResult {

	private final PrologTerm kind;

	JLogTypedResult(PrologProvider provider, PrologTerm name, PrologTerm kind) {
		super(provider, name);
		this.kind = kind;
	}

	JLogTypedResult(PrologProvider provider, String kind, int position) {
